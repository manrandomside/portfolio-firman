...
type AreaActionMenuProps = {
  areaId: string;
  areaTitle: string;
};

export function AreaActionMenu({ areaId, areaTitle }: AreaActionMenuProps) {
  ...
  return (
    <div className="relative inline-block" ref={menuRef}>
      <button
        ...
        aria-label={`Actions for ${areaTitle}`}
        aria-haspopup="menu"
      ...
      {isOpen && (
        ...
        >
          ...
          <Link
            href={`/admin/areas/${areaId}/delete`}
            role="menuitem"
            className="flex items-center gap-2.5 px-3.5 py-2.5 text-sm text-foreground hover:bg-soft transition-colors"
            onClick={() => setIsOpen(false)}
          >
